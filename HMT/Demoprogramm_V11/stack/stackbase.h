...
void StackBase<T, PDI, PDO, SpiSsHndlr>::configurePhy()
{
    ...
    else
    {
        ...
        {
        case 1: mseq = Phy::MSEQ_OD_1; break;
        case 2: mseq = Phy::MSEQ_OD_2; break;
        case 8: mseq = Phy::MSEQ_OD_8; break;
        default:
 ...
 * 
 * The user-provided cyclic code runs inside the application's main loop. It 
 * should repeatedly call \ref canRunUserCode "Stack::instance.canRunUserCode()" 
 * to determine if it may perform any lengthy calculations or otherwise access
 * the stack. This function will return \c true once per IO-Link communication
 * cycle, <b>even in the absence of IO-Link communication</b> when it is set to
 * \c true once per minimum cycle length . When the stack is 
 * communicating, this function will run between two sequences, just after the 
 * completion of the device response.
 * 
